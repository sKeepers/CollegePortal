...
Канонический GitHub workflow описан в `docs/GITHUB_REPOSITORY.md`. Этот документ остается кратким рабочим конспектом по веткам и локальным операциям.

Актуальная политика:

- GitHub repository `sKeepers/CollegePortal` — canonical source;
- `develop` — основная интеграционная ветка;
- `main` — стабильная ветка/release line;
- задачи выполняются в отдельных `feature/*` ветках;
- Issue и Draft PR создаются до merge;
- CI должен быть green до merge;
- release-процесс описан в `docs/RELEASE_PROCESS.md`.

Окружения и пути не дублируются здесь. Используйте `docs/ENVIRONMENTS.md` и `docs/PATH_POLICY.md` как источники истины по DEV/UAT/PROD и рабочим каталогам.
## Основные ветки
