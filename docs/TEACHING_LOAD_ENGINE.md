...
Schedule Engine использует `teaching_load_items` как источник допустимых сочетаний группа-дисциплина-преподаватель. Каждая новая запись расписания может ссылаться на `teaching_load_item_id`, а покрытие нагрузки считается по фактически запланированным занятиям.
