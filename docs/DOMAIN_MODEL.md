...
## DOCS-ENGINE-001: подсистема документов

Добавлен домен печатных форм и выданных документов. Основные сущности:

- `DocumentType` — тип документа и правило нумерации;
- `DocumentTemplate` — версия DOCX-шаблона;
- `GeneratedDocument` — сформированный документ, регистрационный номер, статус и публичный идентификатор проверки;
- `DocumentEvent` — история жизненного цикла документа;
- `DocumentNumberSequence` — годовые последовательности номеров;
- `StudentOrder` — foundation для приказов студента.

Первый документ: `student_enrollment_certificate`, справка, подтверждающая обучение студента. Документы связываются с текущими профилями через `subject_type`/`subject_id`; дальнейшая консолидация с `Person` выполняется эволюционно.
