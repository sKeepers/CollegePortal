...
## Связь с Schedule Engine (ST-002A)

Учебный план задает дисциплины и часы, Teaching Load распределяет их по преподавателям, а Schedule Engine превращает назначенную нагрузку в проверяемые занятия расписания. `curriculum_subjects` не копируются в расписание напрямую, связь проходит через `teaching_load_items`.
