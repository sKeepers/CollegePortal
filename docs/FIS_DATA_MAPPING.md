...
## Загрузка справочников из ФИС

Значения справочников не набиваются руками: ФИС отдаёт их методами 2.8 «получение списка справочников» и 2.9 «получение списка элементов справочника». Отдельного XSD для ответов ФЦТ не публикует, структура описана в спецификации схемами:

```
Dictionaries → Dictionary (1..∞) → Code, Name
             → Error → ErrorCode, ErrorText

DictionaryData → Code, Name
               → DictionaryItems → DictionaryItem (1..∞) → ID, Name
```

**Справочник направлений подготовки — исключение**: запись в нём устроена иначе, и это тот справочник, из которого в портале берутся специальности:

```
DictionaryItem → DirectionID, Name, NewCode, QualificationCode?, UGSCode?, UGSName?, ParentDirectionID
```

| API | Что делает |
| --- | --- |
| `POST /api/fis/dictionaries/preview` | разбирает файл и показывает, что произойдёт; в базу не пишет |
| `POST /api/fis/dictionaries/apply` | применяет: направления → специальности, обычный справочник → сопоставления |

Направления подготовки сопоставляются со специальностями **по коду** (`NewCode` ↔ `specialties.code`): именно он печатается в дипломе. Отсутствующая специальность создаётся, изменившееся название обновляется, а `DirectionID` записывается сопоставлением `fis:DirectionID` вместе с укрупнённой группой в метаданных.

Обычный справочник сопоставляется с элементами справочника портала **по точному названию** после приведения регистра, «ё» и пробелов. Похожие названия не склеиваются — несовпавшие записи возвращаются списком. Куда класть справочник, подсказывает `config/fis_api.php` → `dictionary_intake` (ключ — код справочника из ответа ФИС), но оператор может указать справочник портала явно.

Источник файла сейчас — выгрузка из ФИС или АРМ. Когда на шлюзе появятся учётные данные, тот же разбор получит ответ шлюза: он от источника не зависит.
