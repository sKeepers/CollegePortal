...
## Ручной UAT преподавателя

Все шаги выполняются на DEV: `https://84.54.208.134:5443`. Пароль в отчёты, скриншоты и Issue не включать.

1. Журнал преподавателя:
   - роль: teacher;
   - учётная запись: `teacher@college-portal.local`;
   - URL: `https://84.54.208.134:5443/journal`;
   - действия: открыть `Мои занятия`, выбрать занятие преподавателя, проверить доступность темы, посещаемости и оценок; попытаться открыть занятие другого преподавателя прямой ссылкой, если она известна;
   - ожидаемый результат: отображаются и редактируются только собственные занятия; чужое занятие не раскрывает данные и не допускает изменения; редактор расписания отсутствует.
2. Переход к журналу из расписания:
   - роль: teacher;
   - учётная запись: `teacher@college-portal.local`;
   - URL: `https://84.54.208.134:5443/schedule`;
   - действия: выбрать собственное занятие и нажать `Журнал`;
   - ожидаемый результат: открывается журнал конкретного занятия. Для legacy `ScheduleLesson` без `schedule_entry_id` допускается только отфильтрованный список журнала по дате и преподавателю; результат фиксируется отдельно.
3. Нагрузка преподавателя:
   - роль: teacher;
   - учётная запись: `teacher@college-portal.local`;
   - URL: `https://84.54.208.134:5443/teaching-load`;
   - действия: открыть раздел и проверить отсутствие действий создания, изменения, импорта, экспорта, генерации и назначения преподавателей;
   - ожидаемый целевой результат: видна только собственная нагрузка преподавателя в режиме чтения;
    - статус исправления: self-scoped read-only сценарий реализован в GitHub-ветке и ожидает развёртывания на DEV; до развёртывания возможен `403`. Это не основание выдавать teacher административное permission.
4. Доказательства:
   - записать дату и время, URL, роль, учётную запись, ожидаемый и фактический результат каждого шага;
   - приложить скриншот без пароля, токенов, персональных данных студентов и QR payload;
   - для ошибки сохранить текст ошибки и HTTP status без session cookie и authorization header.
