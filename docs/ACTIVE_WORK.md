...
2. Полоса прокрутки меню: `q-scroll-area` в `AppLayout.vue` меряет высоту раньше, чем grid её раздаёт. Чинить с проверкой в браузере.
3. Проверить импорт сотрудников на реальном файле: колонки «Рабочий график» и «Создать учетную запись» возвращены и покрыты тестами, но на живых данных не прогонялись.
4. Решить судьбу ветки `feature/access-control-foundation`. Документы из неё перенесены, код — нет; сливать нельзя, переделывать инкрементально. Решение за владельцем системы.
