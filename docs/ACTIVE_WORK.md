...
- **`git push` в origin в этой сессии выполнить не удалось: действие заблокировано.** Коммит релиза `88c989bf9`, тег `v0.8.0-rc6` и все четыре коммита ветки существуют **только на DEV**. Это ровно тот риск «работа в единственном экземпляре», из-за которого уже спасали ветки `rescue/*`. Отправить в origin при первой возможности.
- Ветка не влита в `develop` намеренно: CI на неё не прогонялся, потому что push заблокирован. Решение о вливании — за владельцем.
...
1. Отправить в origin `develop`, тег `v0.8.0-rc6` и ветку `feature/access-buildings-muster`, дождаться CI на ветке.
2. Решить, вливать ли ветку в `develop` и собирать ли `0.8.0-rc7`.
