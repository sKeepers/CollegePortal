...
Текущее состояние: частично реализовано через Person foundation. Персональные данные все еще сохраняются в отдельных профилях студентов, преподавателей, абитуриентов и пользователей на переходном этапе.
...
Текущее состояние: частично реализовано. Базовые цифровые QR-пропуска и связь с существующими профилями уже используются; полная нормализация через `Person` и дополнительные носители identity остаются запланированными.
...
Текущее состояние: частично реализовано для QR-пропусков. Печатные/динамические сценарии требуют отдельного smoke-test, UAT и production deployment.
...
Текущее состояние: частично реализовано для мобильного QR и mobile scanner. Production readiness зависит от HTTPS, устройства, браузера и UAT.
...
3. Поддерживать QR/Digital Identity как частично реализованный контур и не считать его production-ready без smoke/UAT.
4. Завершить проверку Mobile Student Cabinet и мобильного QR на реальных DEV/UAT устройствах.
