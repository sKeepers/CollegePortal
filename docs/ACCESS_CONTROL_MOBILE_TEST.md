...
## ACCESS-002.4 mobile scanner regression

The mobile scanner page must render a visible fallback state even when the browser does not provide `BarcodeDetector`. In that case the scanner uses the local `jsQR` fallback and must not upload camera frames to the server.

The secure-context badge must be computed by the Vue component instead of referencing `window` directly from the template. If camera APIs are unavailable, show a Russian error message rather than a blank page.

Xiaomi 11T Pro smoke should verify:

- `/access/mobile-scanner` opens without a white screen;
- HTTPS secure context is shown;
- camera permission prompt is visible;
- either `BarcodeDetector` or `jsQR fallback` is shown;
- denied replay still displays the known synthetic owner and category.
