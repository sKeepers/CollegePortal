...
Основной HTTPS endpoint DEV и рекомендуемая точка входа для ручного UAT:

- `https://192.168.34.114:5443`
- `https://college-dev.local:5443`, если имя прописано в DNS или `hosts`

Frontend автоматически перенаправляет не-localhost HTTP-запросы на HTTPS-порт `5443`. Например:
