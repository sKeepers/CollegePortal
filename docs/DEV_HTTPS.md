...
PROD не затрагивается. Пользовательский вход в DEV должен выполняться только через HTTPS:

- `https://192.168.34.114:5443`
- `https://college-dev.local:5443`, если имя прописано в DNS или `hosts`

HTTP-порт Vite `5174` не публикуется во внешнюю сеть и остается только локальным диагностическим портом на DEV-сервере:

- `http://127.0.0.1:5174`

Frontend дополнительно перенаправляет не-localhost HTTP-запросы на HTTPS-порт `5443`, если такой запрос все же попал в приложение. Например:

```text
http://192.168.34.114:5174/dashboard -> https://192.168.34.114:5443/dashboard
```

Важно: адрес вида `https://192.168.34.114:5174` работать не будет, потому что порт `5174` обслуживает обычный HTTP. В этом случае браузер останавливает соединение до загрузки frontend-кода и показывает `ERR_SSL_PROTOCOL_ERROR`.
...
Контейнер Vite доступен хосту только через `127.0.0.1:5174`. Для телефонов, рабочих станций и ручного UAT используйте только HTTPS proxy `:5443`.

## Сертификаты и доверие браузеров
...
Браузер перестанет показывать предупреждение `Небезопасно` только после установки `college-dev-root-ca.crt` в доверенные корневые центры конкретного устройства. Для полностью автоматического доверия без установки CA нужен публичный домен и публичный сертификат, например через корпоративный DNS и Let's Encrypt. Самоподписанный сертификат для IP-адреса не может стать доверенным у пользователей сам по себе.
