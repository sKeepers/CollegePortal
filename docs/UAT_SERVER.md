...
This section records the INFRA-008 acceptance install and is kept as history. The same machine is now the production server: it runs `0.8.0-rc7`, answers as `https://portal.skki.ru` with a trusted Let's Encrypt certificate, and redirects plain HTTP. See [TLS_CERTIFICATE.md](TLS_CERTIFICATE.md) and [ACTIVE_WORK.md](ACTIVE_WORK.md) for the current state.
