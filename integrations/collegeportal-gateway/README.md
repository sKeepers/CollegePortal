...
Текущий пакет предназначен для установки на отдельный ViPNet-ПК и содержит FIS-adapter для тестового контура ФИС ГИА и Приема. Продуктивный endpoint ФИС в пакете не включается и не должен использоваться без отдельного решения.
...
В нем необходимо заменить `SharedSecret=CHANGE_ME` на реальный секрет, выданный для связи CollegePortal с Gateway.
