...
namespace CollegePortal.Gateway
{
    public class GatewayServer : IDisposable
    {
        ...
        private void Process(HttpListenerContext ctx)
        {
            ...
            if (!payload.IsOk) status = payload.Code == "operation_disabled" || payload.Code == "production_disabled" ? 403 : 502;
            Write(ctx, status, Payload(payload, _config.ServiceVersion));
            _audit.Write(requestId, path, started, (int)sw.ElapsedMilliseconds, payload.IsOk ? "ok" : "failed", payload.Code, payload.Message);
        }
        ...
        private static string Payload(GatewayPayload payload, string version) { return "{\"ok\":" + (payload.IsOk ? "true" : "false") + ",\"error_code\":\"" + Json(payload.Code) + "\",\"message\":\"" + Json(payload.Message) + "\",\"latency_ms\":" + payload.LatencyMs + ",\"gateway_version\":\"" + Json(version) + "\"}"; }
