...
- [x] Устранить массовые повторы ФИО в демо-наборе и связать demo Student/Teacher с `people`.
- [x] Создать demo `employees` для преподавателей, чтобы раздел «Сотрудники» не был пустым.
- [x] Исправить отображение времени посещаемости без UTC-сдвига и обновить demo-события проходной.
- [x] Усилить раннюю валидацию мастера заявления и визуальное состояние шагов.
...
- [x] SEC-002: убрать хранение bearer-токена из frontend `localStorage`, перейти на HttpOnly Secure cookie. **Закрыто 10.08.2026.** Токен уходит в `cp_session` — httpOnly, Secure по факту защищённого соединения, SameSite=Strict; в теле ответа его больше нет. Защита от CSRF привязана к сессии: признак — HMAC от токена под ключом приложения, сервер пересчитывает его из cookie и сравнивает с заголовком `X-CSRF-Token`; проверяются только изменяющие методы. Заголовок `Authorization` оставлен для скриптов — браузер его сам не подставляет. Схема выдачи в базе не менялась, миграции нет. Добавлено скользящее продление: раньше его не было вовсе. **Разблокирует `AUTH-005`.** Осталась ручная проверка в браузере — вход, мобильный сканер, личный пропуск.
...
- [x] SEC-006: ограничитель частоты `api.authenticated` считает не по пользователю, а по адресу. **Закрыто 10.08.2026.** Владельца токена определяет общий `App\Support\Auth\ApiTokenResolver`: его спрашивают и ограничитель, и `api.token`, обращение к базе остаётся одно на запрос. Неопознанный токен считается по адресу — иначе перебор случайных значений давал бы каждому запросу свой счётчик. Порог 120 не менялся. На живом DEV: два человека с одного адреса получают по 119, повторный запрос того же человека — 118, два мусорных токена делят один счётчик. Ниже — исходное описание. В `AppServiceProvider` ключ задан как `$request->user()?->id ?: $request->ip()`, но `ThrottleRequests` в Laravel идёт по приоритету раньше `api.token`, пользователь ещё не разрешён, и ключом всегда оказывается IP. Проверено на стенде: `study@local` получает `x-ratelimit-remaining: 119`, следующий за ним `director@local` — `118`, счётчик один на всех. Снаружи весь колледж приходит через один NAT, и общие 120 запросов в минуту выбираются несколькими одновременно работающими людьми — портал начнёт отдавать 429 всем сразу. Чинить либо порядком middleware, либо разбором токена внутри самого ограничителя.
...
- [x] Сделать Person единственным местом записи ФИО и контактов: карточки сотрудника и преподавателя пишут в него, копия в `teachers` приходит зеркалом.
- [x] Включить `students` в ту же рассылку: правка учебной карточки доходит до человека, СНИЛС намеренно остаётся за карточкой человека.
- [x] Дать разделу «Люди» форму редактирования — единственное место, где общее поле можно очистить.
- [ ] Решить, что делать с копиями ФИО в `graduates` и `applicant_applications`: обе историчны, автоматически переписывать их не стали.
...
## AUTH: расширение способов входа

План с зависимостями и критериями готовности: [docs/AUTH_AND_MOBILE_PLAN.md](docs/AUTH_AND_MOBILE_PLAN.md).

- [x] AUTH-001: смена пароля в личном кабинете для всех ролей. **Закрыто 10.08.2026** вместе с разделом «Моя учётная запись» (`/account`): смена только с подтверждением текущего пароля, новый не короче восьми символов, факт смены в журнале. Там же человек правит свою почту и телефон — через `PersonService`, поэтому исправление доходит до карточек преподавателя и студента. Логин не меняется: это отдельная задача, требующая подтверждения владения адресом.
- [ ] AUTH-002: восстановление пароля. Требует решения по SMTP и по пользователям без email.
- [x] AUTH-005: общий слой внешних провайдеров и таблица `user_identities`. **Закрыто 10.08.2026.** Два ограничения уникальности: внешний аккаунт не принадлежит двум учётным записям, у человека один аккаунт на провайдера. Контракт `ExternalIdentityProvider`, реестр (пока пустой — провайдеры придут с `AUTH-003` и `AUTH-004`), `ExternalIdentityService`, блок «Способы входа» в `/account`, отвязка администратором. Привязка и отвязка требуют текущего пароля; отказ по занятому аккаунту не сообщает, чей он; создать учётную запись через провайдера нельзя по устройству — метода такого нет.
- [x] AUTH-003: вход через Telegram. **Код закрыт 11.08.2026**, `TelegramLoginProvider` поверх слоя `AUTH-005`: подпись `HMAC-SHA256` ключом `SHA256(токен бота)` считается на сервере, обращений к Telegram нет, ответ виджета живёт пять минут. Открытые маршруты `auth/providers` и `auth/provider-login`, ограничитель `auth.external` — 30 в минуту по адресу. Без ключей в `.env` способ входа не подключается вовсе. **Живая проверка не проведена и до релиза невозможна:** Login Widget работает только на `portal.skki.ru`, у стенда самоподписанный сертификат. Порядок проверки — в [docs/TELEGRAM_BOT_SETUP.md](docs/TELEGRAM_BOT_SETUP.md).
- [ ] AUTH-004: вход через MAX. Кнопки «Войти через MAX» для внешнего сайта не существует — проверено 10.08.2026, официальное API описывает ботов и mini-apps. Остаётся подтверждение через бота: человек вводит логин, портал показывает код, бот просит подтвердить. Бот MAX всё равно появляется ради `NOTIFY-001`, поэтому задача сводится к провайдеру поверх него.
- [ ] DORM-001: модуль общежития. Предложен владельцем 11.08.2026, **объём определён его ответами в тот же день**; первый шаг — разбор, не код. Комендант общежития ведёт комнаты и места, заселение и выселение, оплату, происшествия, ночные отсутствия. Заместитель директора по воспитательной работе — работу с трудными, социальный паспорт, прописку, рекомендации кого куда переселить и кого подселить, фиксацию провинностей. Проживающий отмечается признаком в карточке студента. Проход связывается с проживанием: владелец хочет видеть, кто ночует; общежитие на Серова, 277 заводится третьим корпусом с одной точкой прохода. Права обеих ролей заводятся миграцией, а не только сидером. Четыре вещи требуют решения в разборе: определение «ночного отсутствия», разграничение доступа к социальному паспорту и работе с трудными, судьба записи о провинности, глубина учёта оплаты. Реальные ФИО сотрудников в репозиторий не переносятся: они заводятся в системе через кадры.
- [ ] AUTH-006: ряд кнопок на странице входа — Яндекс ID и VK ID поверх слоя `AUTH-005`. **Состав закрыт решением владельца 11.08.2026:** Telegram, Яндекс ID, VK ID и MAX подтверждением. Sber ID, T‑ID, Госуслуги (ЕСИА), Google и Apple отброшены — не рассматривать и не предлагать заново. От владельца нужна регистрация приложения у каждого провайдера: `client_id` и секрет ложатся в `.local/secrets/`, ключи `.env` — `YANDEX_OAUTH_CLIENT_ID`, `YANDEX_OAUTH_CLIENT_SECRET`, `VK_OAUTH_CLIENT_ID`, `VK_OAUTH_CLIENT_SECRET`.

Требование безопасности для AUTH-003, AUTH-004 и AUTH-006: вход через внешний сервис только привязывается к существующей учётной записи и никогда не создаёт новую.

## MOB: мобильный контур для пяти ролей

Реализован кабинет студента. Роль `curator` уже существует в `RoleSeeder`.

- [x] MOB-001: мобильный кабинет преподавателя. Сделано 10.08.2026, `01b1ed77d`: свои занятия на день и неделю, состояние журнала по каждому, отметка посещаемости и оценки в один экран без горизонтальной прокрутки, свой QR-пропуск. Право `mobile.teacher.view` заведено `ARCH-001` 10.08.2026 (`9bc917864`) и объявлено у маршрута `GET api/mobile/teacher`; выдано ролям `teacher` и `curator`. Заодно заведены `mobile.curator.view` и `mobile.admin.view` под `MOB-002` и `MOB-004`, и закрыт правом `mobile.student.view` маршрут `GET api/mobile/student` — серверной проверки у него не было вовсе.
- [x] MOB-002: мобильный кабинет куратора. Сделано 11.08.2026: закреплённые группы, посещаемость за день и неделю через `AttendanceAnalysisService`, список студентов с контактами, события проходной по своей группе. Доступ к чужой группе закрыт `groups.curator_id` и покрыт `MobileCuratorAccessTest` — восемь случаев, включая куратора, который ведёт занятие в чужой группе.
- [x] MOB-004: мобильный кабинет администратора. Сделано 11.08.2026: показатели колледжа и сегодняшнего дня, входящие, решение по запросу на переоткрытие журнала прямо с телефона, поиск человека с переходом в карточку. Список входящих не написан второй раз — он собирается общим `frontend/src/services/adminInbox.js`, которым теперь пользуется и «колокольчик» на десктопе.
- [~] MOB-003: мобильное рабочее место проходной. **Сделано 11.08.2026 всё, что делается без боевого домена:** компактный режим, результат поверх кадра, разбор центральной области вместо полного кадра, устойчивость к сворачиванию и блокировке экрана, Wake Lock, распознавание отобранной камеры. Нормализация русской раскладки проверена сквозным запросом. **Закрыть задачу может только проверка на телефоне по `https://portal.skki.ru`** — камеру мобильный браузер на самоподписанном сертификате не даёт вовсе. Список из двенадцати пунктов для той проверки — [docs/MOB_003_FIELD_CHECKS.md](docs/MOB_003_FIELD_CHECKS.md).
- [~] MOB-005: PWA-упаковка. **Отменена владельцем 10.08.2026.** Технически разблокирована (`SEC-004` и `SEC-002` закрыты), но упаковка отложена в пользу содержимого: на телефоне полноценно живёт только студент. Разбор путей сохранён в `docs/MOBILE_APP_PLAN.md`.

SEC-004 блокирует четыре из девяти задач этих двух линий, поэтому его приоритет выше, чем предполагалось ранее.
...
- [x] Получить официальные XSD и спецификацию 4.9 — лежат в `docs/external-services/ФИС ГИА и Приема/`
- [x] Собрать `PackageData` по официальной XSD 4.9 и снять `pending-official-spec`
- [ ] Получить WSDL и подключить реальные SOAP methods строго по спецификации
- [ ] Заполнить сопоставления справочников ФИС и UID конкурсных групп
...
- [x] Получить официальные документы ФЦТ 4.9 и сформировать SHA-256 manifest
- [x] Реализовать XML-контракт строго по официальной XSD (метод импорта, элемент `PackageData`)
- [ ] Реализовать SOAP-конверт по WSDL, когда он будет получен
...
- [ ] HR-002: у отдела кадров нет выхода, когда на одного человека находится несколько совпадений. `HrService::resolvePerson` в этом случае отвечает «Найдено несколько возможных Person. Выберите существующую запись вручную», а выбрать кадры не могут: реестр людей закрыт им намеренно — там же студенты и абитуриенты. Нужен либо отдельный поиск по людям в кадровой области видимости, либо разрешение конфликта прямо в карточке сотрудника.

## Демонстрационные данные

- [ ] DEMO-001: сделать демонстрационный набор правдоподобным. Владелец смотрел нынешний и отказался показывать его на демонстрации: слишком много двойников, повторяющиеся фамилии и никакого правдоподобного движения людей.
  - ФИО сейчас собираются перебором десяти фамилий, десяти имён и десяти отчеств по остатку от деления (`DemoDataSeeder::seedStudents`), поэтому на 600 студентов приходится 10 фамилий, и каждая встречается 60 раз. Нужен словарь на несколько сотен фамилий с учётом рода, чтобы совпадения были редкими и выглядели естественно.
  - События проходной генерируются одинаковым шаблоном на всех. Правдоподобное движение — это разное время прихода по группам и сменам, часть людей без входа, часть с выходом и повторным входом, редкие ночные и выходные события.
  - Оценки и посещаемость распределены равномерно; в жизни у группы есть отстающие, отличники и провалы по отдельным дисциплинам.
  - Проверять на объёме: набор в 600 студентов не должен строиться минутами.

## Студенты

Замечания владельца системы от 09.08.2026 по разделу `/students`.

- [x] GUI-018: добавить в фильтры раздела «Студенты» «Курс» (год обучения) и «Специальность». **Сделано 10.08.2026.** Курс и специальность фильтруются через связь с группой, отдельных полей в `students` не заведено. Значения выпадающих списков собираются из уже загруженных групп, а не из `api/specialties`, — права на справочники для них не нужны. Заодно `GroupController::index` научился отдавать больше одной страницы: на двадцати строках половина групп в списки не попадала.
...
- [x] BUG-010: в фильтрах раздела «Студенты» выпадающий список «Статус» пуст, нажатие ничего не показывает. **Сделано 10.08.2026.** Причина подтвердилась: `api/admin/reference` целиком требовал `reference.manage`, а у ролей «Учебная часть» и «Учебная часть 2» его нет. Чтение справочников отделено от правки правом `reference.view`, оно выдано всем ролям — сидером для новых установок и миграцией для существующих. Симптом вылечен отдельно: справочник теперь помнит, чем кончилась загрузка, и подпись под списком говорит, нет ли прав, пуст ли справочник или запрос не прошёл.
