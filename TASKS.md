...
- [ ] Включить `students` в ту же рассылку: карточка студента хранит собственную копию ФИО, и правка в ней до человека не доходит.
- [ ] Дать разделу «Люди» форму редактирования: очистить общее поле сейчас можно только запросом `PATCH /api/people/{id}`.
