...
## Актуальное состояние и следующий фокус

Старый ближайший фокус GUI-015 / QR-002 / MOB-001 больше не отражает фактическое состояние проекта. Эти направления были расширены последующими задачами и частично закрыты отдельными foundation-работами.

### Статус направлений

| Направление | Статус | Комментарий |
| --- | --- | --- |
| GUI foundation | реализовано | Quasar GUI, layout, UI Foundation, Design System и route lazy loading уже зафиксированы в проекте. |
| Students и Groups | реализовано | Основные разделы контингента работают в новом GUI и связаны с Workspace/RBAC. |
| RBAC foundation | реализовано | Permission matrix и permission-aware frontend UI реализованы; точечные permissions развиваются по мере модулей. |
| Access Control и QR-related foundation | частично реализовано | QR/CP1/HID/mobile scanner и access-control foundation требуют физического smoke-test, UAT и отдельного production deployment. |
| DEMO-001 | выполнено | Demo-readiness login/dashboard/mobile/security fixes завершены. |
| DEMO-001.1 | выполнено | DEV helper входа и печатная карточка учетной записи завершены. |
| DEMO-002 | code completed | Код демонстрационной базы завершен и запушен; Issue/PR были заблокированы отсутствием GitHub write-доступа в той среде. |
| DOCS-001 | выполнено | Создан единый центр управления документацией и включен в develop перед DOCS-002. |

### Следующий крупный этап

Следующий крупный функциональный этап еще должен быть утвержден. До утверждения новые roadmap-записи не должны помечаться выполненными без подтвержденного commit/PR/merge.

### Плановые контуры

- Access Control: физический smoke HID/mobile scanner, UAT, production deployment и закрытие legacy compatibility plan.
- Demo: проверка демонстрационной базы на DEV и ручной показ руководству.
- FIS/GIA: продолжение только по официальным контрактам и TEST-контуру, без production endpoint.
- Documentation: регулярная сверка `docs/PROJECT_STATUS.md`, `TASKS.md`, `ROADMAP.md`, `CHANGELOG.md` и GitHub Project.
