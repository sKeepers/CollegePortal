...
class SubgroupsAreSkippedNotRefusedTest extends TestCase
{
    ...
    /**
     * Порядок настоящий: сперва проверка ошибок, потом загрузка.
     *
     * Этот тест закрывает дыру, которую вскрыло слияние с правкой соседней области.
     * Первые два теста зовут `import()` напрямую — и не заметили бы, что подгруппа с
     * **той же аудиторией** отказывает раньше, чем дойдёт до пропуска: занятие клетки
     * дало бы «аудитория занята» про себя же, и строка стала бы ошибкой, а не
     * пропуском. Порядок вызовов здесь такой же, как в `UniversalImportService`.
     */
    public function test_a_subgroup_in_the_same_room_reaches_the_skip_and_is_not_refused(): void
    {
        $context = $this->context();
        $handler = app(ScheduleImportHandler::class);
        $handler->import($this->row($context), 'create');

        $second = $this->row($context, ['teacher_name' => 'Кузнецов Кирилл Кириллович']);

        $this->assertSame([], $handler->businessValidationErrors($second),
            'занятие своей же клетки не должно давать строке ни одной ошибки');
        $this->assertSame('skipped', $handler->import($second, 'create'));
        $this->assertSame(1, ScheduleLesson::count());
    }
