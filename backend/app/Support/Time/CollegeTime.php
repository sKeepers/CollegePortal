...
final class CollegeTime
{
    ...
    /**
     * Дата колледжа, которой принадлежит эта отметка времени.
     *
     * Нужна там, где день события сравнивают с днём человека: `toDateString()`
     * у самой отметки даёт дату **по UTC**, и проход в начале первого ночи
     * оказывается вчерашним. Коварство в том, что отбор при этом может быть уже
     * верным — строка приходит из базы и тут же отбрасывается сравнением, а
     * границы выглядят починенными.
     *
     * Наступили на этом 28.08.2026 в `AttendanceAnalysisService::sessionPairs`.
     */
    public static function dateOf(CarbonInterface|string $moment): string
    {
        return self::dateString($moment);
    }

    /**
     * Момент занятия: дата плюс время «на стене».
     *
     * Расписание хранит `starts_at` как время суток — то, что написано в сетке
     * звонков и что человек видит на экране: 08:30 значит половину девятого утра
     * в колледже. Проходная пишет `event_time` в UTC. Пока эти два склеивали
     * наивно, сравнение шло между половиной девятого **UTC** и настоящим
     * временем прохода: опоздания и ранние уходы получались со сдвигом в три
     * часа у каждой строки, а вошедший за десять минут до пары числился
     * пришедшим за два часа пятьдесят.
     *
     * Замечание для чтения: `lesson_date` — колонка типа `date`, часового пояса
     * у неё нет и переводить её не надо; переводится только склейка даты со
     * временем суток, потому что её сравнивают с меткой времени.
     */
    public static function moment(CarbonInterface|string $date, string $time): CarbonImmutable
    {
        return CarbonImmutable::parse(self::dateString($date).' '.$time, self::ZONE)->utc();
    }

    /**
     * Та же отметка времени, выраженная по часам колледжа — для показа.
     *
     * Приложение хранит время в UTC и правильно делает: два года записей лежат
     * в одном поясе, и трогать `app.timezone` нельзя. Но **на бумагу и на экран
     * час обязан идти местный**: человек прошёл в 09:15 и ждёт в ведомости
     * 09:15, а не 06:15.
     *
     * Заведено 30.08.2026, когда нашлось, что отчёт проходной **отбирает**
     * события по суткам колледжа, а **печатает** их время в UTC — два календаря
     * внутри одной ручки. Проверка этого не видела: приспособление сеяло
     * событие тоже в UTC, обе стороны ошибались одинаково, и утверждение
     * выглядело верным.
     *
     * Отбор и показ — разные вещи, и путать их дорого: отбор решает, попадёт ли
     * строка в документ, показ — что в ней написано. Первое чинит `dayRange`,
     * второе этот метод.
     */
    public static function forDisplay(?CarbonInterface $moment): ?CarbonImmutable
    {
        return $moment === null
            ? null
            : CarbonImmutable::parse($moment)->setTimezone(self::ZONE);
    }
