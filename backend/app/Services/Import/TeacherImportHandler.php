...
class TeacherImportHandler extends AbstractImportHandler
{
    ...
    public function businessValidationErrors(array $data): array
    {
        ...
        // Число вместо части имени — это не имя, а пометка выгрузки. В кадровой
        // выгрузке CARDDEX 28.08.2026 у семи строк вместо отчества стояли «1»,
        // «2», «3», и означали они не людей, а **сколько карт у одного человека**.
        // Загрузка приняла их за отчества и завела семь карточек вместо трёх;
        // разбирать это пришлось слиянием, задним числом.
        //
        // Строка останавливается целиком: догадаться, что имел в виду источник,
        // портал не может, а завести лишнего человека — может.
        foreach (['last_name' => 'Фамилия', 'first_name' => 'Имя', 'middle_name' => 'Отчество'] as $field => $label) {
            if (preg_match('/^\d+$/', trim((string) ($data[$field] ?? '')))) {
                $errors[$field] = [$label.' записано числом. Так выгрузка кадров помечает, что у человека несколько карт: загрузка карт такую строку берёт намеренно, а карточку человека по ней заводить нельзя — впишите отчество или уберите цифру.'];
            }
        }
