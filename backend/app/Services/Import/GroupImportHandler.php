...
class GroupImportHandler extends AbstractImportHandler { use ResolvesImportRelations;
  ...
  */
 public function fields(): array { return ['name'=>['label'=>'Название','required'=>true,'aliases'=>['группа','название','name']],'specialty'=>['label'=>'Специальность','required'=>true,'aliases'=>['специальность','specialty']],'course'=>['label'=>'Курс','required'=>true,'aliases'=>['курс','course']],'year_start'=>['label'=>'Год набора','required'=>true,'aliases'=>['год набора','year_start']],'education_program_id'=>['label'=>'ID программы','required'=>false,'aliases'=>['education_program_id']],'education_program_name'=>['label'=>'Программа','required'=>false,'aliases'=>['программа','образовательная программа','education_program']],'curator_id'=>['label'=>'ID куратора','required'=>false,'aliases'=>['curator_id']],'curator_name'=>['label'=>'Куратор','required'=>false,'aliases'=>['куратор','curator']]]; }
 ...
 public function rules(): array { return ['name'=>['required','string','max:255'],'specialty'=>['required','string','max:255'],'education_program_id'=>['nullable','integer','exists:education_programs,id'],'curator_id'=>['nullable','integer','exists:teachers,id'],'course'=>['required','integer','min:1','max:6'],'year_start'=>['required','integer','min:2000','max:2100']]; }
