...
class EmployeeImportHandler extends AbstractImportHandler
{
    ...
    /**
     * Обработчик принимает и проверяет дату рождения и СНИЛС, а в шаблоне их не
     * было — и выгрузка сотрудников, которая берёт колонки отсюда, оба поля
     * теряла. Файл выглядел полным, не будучи им: выгрузка «чтобы был файл»
     * перед очисткой данных оставляла бы эти поля только внутри дампа.
     * Обязательными они не становятся, колонки сопоставляются по синонимам,
     * поэтому старые файлы грузятся по-прежнему.
     */
    public function templateHeaders(): array
    {
        return ['Табельный номер', 'Фамилия', 'Имя', 'Отчество', 'Дата рождения', 'СНИЛС', 'Email', 'Телефон', 'Подразделение', 'Должность', 'Дата приема', 'Статус', 'Занятость', 'Ставка', 'Преподаватель', 'Рабочий график', 'Создать учетную запись'];
    }

    public function templateExample(): array
    {
        return ['', 'Примерова', 'Александра', 'Сергеевна', '15.03.1990', '12345678901', 'employee@example.test', '+70000000000', 'Учебная часть', 'Методист', '01.09.2026', 'Активен', 'Полная занятость', '1', 'Да', 'Пн–Пт, 09:00–18:00', 'Нет'];
    }
