    ...
    public static function definitions(): array
    {
        return [
            ...
            'dorm' => [
                ...
                // Пишется расчётом ночей, а не человеком: по какую ночь он
                // доведён. Без неё посчитанная ночь без отсутствий
                // неотличима от непосчитанной, и сводка врёт уверенно.
                'absences_calculated_through' => ['value' => '', 'type' => 'string', 'is_public' => false, 'label' => 'Ночи посчитаны по', 'description' => 'Заполняется пересчётом ночных отсутствий. Пусто — расчёт ни разу не запускался.'],
            ...
            'certificates' => [
                ...
                // Шапка бланка. Открыты для чтения намеренно: это то, что и так
                // напечатано на каждом выданном документе, а собирает бланк
                // браузер. Название и контакты колледжа сюда не дублируются —
                // они уже есть в группе `general`, и второе место для одного и
                // того же значения однажды разойдётся молча.
                'founder' => ['value' => 'Министерство культуры Ставропольского края', 'type' => 'string', 'is_public' => true, 'label' => 'Учредитель', 'description' => 'Первая строка шапки печатного бланка справки.'],
                'short_name_line' => ['value' => '(ГБПОУ СК «СККИ»)', 'type' => 'string', 'is_public' => true, 'label' => 'Краткое наименование в шапке', 'description' => 'Как краткое имя набрано в шапке бланка, со скобками и кавычками.'],
                'name_genitive' => ['value' => 'ГБПОУ СК «Ставропольского краевого колледжа искусств»', 'type' => 'string', 'is_public' => true, 'label' => 'Название в родительном падеже', 'description' => 'Подставляется в оборот «является студентом N курса …».'],
                'requisites' => ['value' => 'ИНН/КПП 2636019138/263601001   ОГРН 1022601971590', 'type' => 'string', 'is_public' => true, 'label' => 'Реквизиты в шапке', 'description' => 'Строка ИНН/КПП и ОГРН под контактами.'],
                'director_name' => ['value' => 'Т.В. Горбачева', 'type' => 'string', 'is_public' => true, 'label' => 'Подпись директора', 'description' => 'Кем подписывается справка. Директор меняется, поэтому это настройка, а не строка в коде.'],
            ],
            'graduation' => [
