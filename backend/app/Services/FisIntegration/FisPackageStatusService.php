...
class FisPackageStatusService
{
    public function refresh(FisOutboundPackage $package, bool $mock = false): array
    {
        ...
        $package->update([
            'status' => $status,
            'completed_at' => $status === 'completed' ? now() : $package->completed_at,
            'failed_at' => in_array($status, ['failed', 'rejected'], true) ? now() : $package->failed_at,
        ...
        $package->events()->create([
            ...
            'metadata' => ['status' => $status, 'error_code' => $result->errorCode],
            'user_id' => auth()->id(),
        ]);
