...
class EducationDocumentService
{
    ...
    /**
     * В строке названо только учебное заведение, без серии и номера аттестата.
     *
     * Такой документ сегодня **не создаётся**, и это осознанно: аттестат без
     * серии и номера в ФРДО не пройдёт. Но и **молчать об этом нельзя**.
     *
     * 22.08.2026 контингент заводили из списка учебной части, где графа
     * «Учебное заведение» заполнена у **580 строк из 593**, а колонок серии и
     * номера аттестата нет вовсе — школу в списке пишут, реквизиты аттестата
     * лежат в бумажном деле. Все 580 названий прошли через загрузку и исчезли:
     * `syncForPerson` вернула `null`, загрузчики возвращённое не смотрели, и на
     * стенде осталось **ноль** документов об образовании. Разбор личного дела
     * полтора месяца объяснял этот ноль тем, что «данных нет ни в одном
     * источнике». Данные были.
     *
     * Поэтому случай назван отдельно от «нет вообще ничего»: загрузчик обязан
     * сосчитать такие строки и сказать о них человеку. Образец правильного
     * поведения уже есть — `EducationDocumentImportHandler` отказывает громко,
     * с сообщением у поля.
     *
     * **Создавать документ из одного названия школы — решение владельца**, оно
     * ещё не принято (вопрос 1 в `docs/STUDENT_PERSONAL_FILE.md`). До ответа
     * поведение прежнее, изменилось только молчание.
     *
     * @param array<string, mixed> $source
     */
    public static function isOrganisationOnly(array $source): bool
    {
        $has = static fn (string $key): bool => trim((string) ($source[$key] ?? '')) !== '';

        return $has('document_organization') && ! $has('series') && ! $has('number');
    }
