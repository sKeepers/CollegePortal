...
class DormPlacementService
{
    ...
    private function assertRoomTakesOneMore(DormRoom $room): void
    {
        ...
        // Нулевая вместимость — это «мест нет», а не «мест сколько угодно».
        // Условие `capacity > 0 &&` выключало проверку целиком, и комната с
        // нулём принимала кого угодно без счёта: отчёт заселённости показывал
        // бы «мест 0, занято 5, свободно 0». Ноль здесь достижим — он стоит
        // умолчанием у колонки в базе, и правила ввода его разрешают
        // (`min:0`). А отбор «только со свободными местами» ту же комнату
        // уже считал без мест: сравнение `capacity > occupied` при нуле
        // ложно. То есть две части одной возможности расходились — и
        // расходились молча.
        if ($room->capacity <= 0) {
            throw ValidationException::withMessages([
                'dorm_room_id' => "В комнате {$room->number} мест нет: вместимость 0. Если в ней живут, укажите вместимость в карточке комнаты.",
            ]);
        }
