...
class RoleSeeder extends Seeder
{
    ...
    private function permissions(): array
    {
        return [
            ...
            ['module' => 'Teachers', 'code' => 'teachers.bulk_accounts', 'name' => 'Преподаватели: массовая выдача учетных записей', 'description' => 'Создание учетных записей преподавателям разом с одноразовым показом паролей.'],
            ...
            ['module' => 'Graduation', 'code' => 'graduation.gia_protocols', 'name' => 'Выпуск: протоколы ГИА', 'description' => 'Ведение протоколов государственной итоговой аттестации и решений по выпускникам.'],
            ...
            ['module' => 'Students', 'code' => 'certificates.view', 'name' => 'Справки студентам: просмотр', 'description' => 'Просмотр реестра выданных справок и печать реестра.'],
            ['module' => 'Students', 'code' => 'certificates.manage', 'name' => 'Справки студентам: выдача', 'description' => 'Выдача справки студенту, печать бланка, отметка о получении.'],
            ...
            // Право открывает **разделы** «Специальности» и «Образовательные программы»
            // в меню, а не данные: читать их разрешает `reference.view`, выданное всем,
            // и те же сведения портал отдаёт публичной странице вообще без входа.
            // Поэтому у права нет своего маршрута: оно говорит «этой роли эти два
            // раздела нужны по работе», и решает, показывать ли их.
            ['module' => 'System', 'code' => 'reference.programs.view', 'name' => 'Специальности и программы: раздел', 'description' => 'Показывать разделы «Специальности» и «Образовательные программы» тем, кто ведёт контингент и выпуск.'],
    ...
    private function studySchedulePermissions(): array
    {
        return [
            ...
            'attendance.view',
            // Расписание строится на учебном плане, а план — на образовательной
            // программе. Раздел нужен, чтобы посмотреть срок обучения и форму.
    ...
    private function studyRecordsPermissions(): array
    {
        return [
            ...
            'students.bulk_group', 'students.bulk_status', 'students.bulk_course', 'students.bulk_education', 'students.bulk_passes', 'students.bulk_accounts', 'teachers.bulk_accounts', 'students.bulk_archive', 'students.bulk_export',
            ...
            'graduation.view', 'graduation.gia_protocols', 'graduation.edit', 'diploma.blanks.view', 'diploma.blanks.manage', 'certificates.view', 'certificates.manage', 'frdo.view',
            'exams.view', 'curricula.view', 'schedule.view', 'teachingload.view',
            'teachers.view', 'subjects.view', 'classrooms.view',
            'trash.request',

            // Решение владельца 17.08.2026. Роль ведёт контингент, и данные для него
            // приходят из ФИС: справочники ведёт учебная часть, а студент появляется
            // только после зачисления заявления в группу. Без этих прав человек видел
            // раздел «Студенты», но завести в нём никого не мог — за каждой загрузкой
            // и за каждым зачислением приходилось идти к администратору.
            //
            // Справочники ФИС применять можно, а **пакеты в государственную систему
            // формировать и отправлять — нет**: это осталось у администратора.
            'fis.view', 'fis.outbound.view', 'fis.settings.manage',

            // Приём — только чтение плюс зачисление. Заводить и править заявления,
            ...
            // Видеть разделы «Специальности» и «Образовательные программы».
            'reference.programs.view',

            // Заявления приходят книгой Excel из личного кабинета ФИС, и загружает
            // их та же роль, что ведёт контингент. Очистка рабочих данных сюда не
            // входит: она разведена с импортом и живёт под `demo_data.manage`.
            'import.manage',
        ];
    }
    ...
    {
        return [
            'dashboard.view', 'people.view', 'students.view', 'students.create', 'students.update', 'students.bulk_group', 'students.bulk_status', 'students.bulk_course', 'students.bulk_education', 'students.bulk_passes', 'students.bulk_accounts', 'teachers.bulk_accounts', 'students.bulk_archive', 'students.bulk_export',
            'groups.view', 'groups.create', 'groups.update',
            ...
            'teachingload.view', 'teachingload.edit', 'teaching_load.generate', 'teaching_load.assign', 'teaching_load.bulk_assign', 'teaching_load.view_coverage', 'exams.view', 'exams.edit', 'graduation.gia_protocols',
            'graduation.view', 'graduation.edit', 'diploma.blanks.view', 'diploma.blanks.manage', 'certificates.view', 'certificates.manage', 'people.update', 'people.link', 'frdo.view', 'fis.view', 'fis.outbound.view', 'fis.outbound.create', 'fis.outbound.generate', 'fis.outbound.validate', 'fis.outbound.send_test', 'fis.outbound.status', 'reference.manage', 'import.manage',
