...
// Изменения расписания — окном в 15 минут. Период запуска и ширина окна в команде
// обязаны совпадать: разойдутся — правки либо продублируются, либо провалятся между
// запусками. Наблюдателя на модели нет намеренно: загрузка расписания на семестр
// создаёт полторы тысячи занятий разом и превратилась бы в бурю сообщений.
Schedule::command('notifications:schedule-changes')->everyFifteenMinutes()->withoutOverlapping();
