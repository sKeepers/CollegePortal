...
- `frontend/src/stores` — Pinia stores: `auth`, `students`, `groups`, `schedule`, `journal`, `search`, `workspace`.
...
- на этапе GUI-006 глобальный поиск работает по студентам и группам, хранит историю запросов в `localStorage` и открывает карточки через `/students?selected=<student_id>&search=<text>` и `/groups?selected=<group_id>`.
...
- добавить журнал обмена с внешними сервисами;
- добавить экспорт Excel/PDF для отчетов;
- подготовить интеграцию с Moodle.

Важно: пользователь временно отложил дальнейшие работы по разделу «Абитуриенту» и приемной комиссии. Текущий фокус после этого — основной контингент, прежде всего карточка студента и будущие поля для ФРДО.
...
### Этап 9. Улучшение интерфейса

- карточка студента;
- карточка абитуриента позже;
- единый рабочий стиль, близкий к 1С-Колледж;
- улучшение расписания и журнала.
...
- расширенные персональные данные студента;
- документы об образовании;
- выпуск;
- шаблон и журнал выгрузок ФРДО.

### Этап 12. ФИС ГИА/Приема
...
### Этап 13. Person, Digital Identity и присутствие

- центральная сущность Person;
- роли Student, Teacher, Employee, Applicant, Guest, Alumni;
- Digital Identity: QR, мобильный QR, печатный QR, фото, срок действия и статус;
- проходная и учет входа/выхода;
- отчеты по присутствию студентов, преподавателей, сотрудников и гостей;
- связь с расписанием, журналом, Dashboard, уведомлениями и аналитикой;
- будущие этапы: NFC, распознавание лиц, IP-камеры, AI-анализ опозданий и отсутствий.

### Этап 14. Доменная архитектура платформы

- зафиксировать основные домены CollegePortal Platform: Identity, Academic, Learning, Administration, Integrations, Analytics;
- использовать `Person` как центральную концепцию идентичности человека;
- разделять справочники, учебный процесс, административные процессы, интеграции и аналитику;
- развивать новые модули внутри понятных доменных границ;
- использовать доменную модель как ориентир при проектировании Service Layer, API и будущих таблиц.
