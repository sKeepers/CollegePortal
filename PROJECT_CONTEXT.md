...
Финальная стабилизация UAT-002.1 выполняется в ветке `feature/uat-002-1-final-stabilization` и не затрагивает PROD. Основной фокус: понятная inline-валидация мастера `/admissions/foundation`, паспортные и адресные поля в сценарии создания заявления, удаление технических терминов из демонстрационного UI, ролевые Dashboard без лишних demo-виджетов, общий splitter для master-detail рабочих мест, единый источник version/build metadata и проверка существующих QR-пропусков для student/teacher на DEV.
