...
Permissions now come with migrations rather than only with the seeder, because the seeder runs at installation and never again. An installed system therefore gains them on update — including two that had been missing from installed systems without anybody noticing.

### Added

- The portal fits in a hand. Four mobile cabinets — teacher, curator, administrator, and the gate workstation — replace the desktop screens that a phone could open but nobody could use: the teacher marks a journal one-handed, the curator sees only the groups they actually curate, the administrator decides a journal reopening request from the same inbox the desktop bell reads, and the scanner survives a locked screen and a lost focus rather than quietly stopping between two people. Whose group a curator may see is decided by `groups.curator_id` inside every endpoint, not by holding the permission that shows the section, and a test walks the cabinet's routes by list so the next route added without that check fails the build instead of opening somebody else's group.
- Signing in with Telegram, standing on a provider layer built before it so that MAX does not arrive with a second way of storing the same thing. The signature is verified here — HMAC-SHA256 over the data-check-string keyed by the binary SHA256 of the bot token, not by the token itself, which is the usual mistake and would make the check prove nothing — and nothing on the sign-in path calls Telegram, so sign-in does not break when somebody else is down. A messenger account is only ever linked to an account that already exists: the service has no method that could create a user. The widget itself can only be verified after this release is installed, because it works solely on the domain registered with the bot and over publicly trusted HTTPS.
- Every role has a page about its own account. Until now the only personal page in the portal belonged to students, and a teacher's password was changed by an administrator. Contacts written there go to the person, not to the login, so the correction reaches the teacher card and the student card at once.
- Deletion goes through the administrator. Whoever keeps a card marks it for deletion with a reason; the administrator sees the queue in the same bell as everything else, and approving moves the card to a trash that can put it back. Emptying the trash is confirmed separately, because after it there is no way back. Groups are deliberately not trashed: students point at them, and a softly deleted group would stop being findable by its own students.
- A demonstration set that looks like a college rather than like a formula, and five sections that used to open empty — journal, curricula, teaching load, hour coverage and graduates — now have content in it. Attendance is generated once and read from the same rows by the journal and by the reports, because a stand where the two disagree teaches the wrong thing about the portal.
- The personnel officer gets the people their own input found. Hiring somebody the portal had seen before ended in a demand it could not satisfy — pick the existing record by hand — from a register personnel deliberately cannot open, since it holds students and applicants too. The answer now returns only the matches their own input produced, and only what tells those people apart, including which field matched: a shared telephone reads very differently from a shared SNILS.
- The FIS package is built from the official 4.9 schema, the dictionaries are loaded instead of typed, and a competition stopped being a property of a programme — FIS keeps it inside the admission terms, so a programme admitting both on budget and on payment has more than one and could not be mapped at all.
- The four reports the menu had been promising since rc2 open.

### Fixed

- The session token is out of reach of JavaScript. It lived in `localStorage`, where any cross-site script took the whole session with it — the one thing that stood between this portal and real personal data. It now travels in an httpOnly `SameSite=Strict` cookie, `Secure` by the fact of a protected connection rather than unconditionally, which would have broken installations running on plain HTTP. The CSRF token is derived from the session token itself rather than stored beside it: whoever cannot read the cookie cannot forge the header, and there is nothing extra to keep. Sessions now extend themselves while somebody works instead of expiring in the middle of the day.
- A permission is declared at the route it protects. It used to be derived from the URL prefix and the method through a table nobody could see from the route, so what a role could do was written in two places that drifted. Reconciling them turned up the real finding: the permission catalogue was a fourth list nobody checked against the other three, and two permissions existed only in the seeder — absent from every installed system, silently skipped when rights were granted. A route in the closed part with no permission declared now fails the build.
- API requests are counted per person, not per address. The limiter ran before anything identified the caller, so the key quietly fell back to the address, and a college leaving through one gateway shared a single budget of requests between everybody in it.
- The direction of a pass alternates only on allowed events. A denial is not a pass — the person stayed on the same side of the door — but while denials counted, one extra scan flipped the direction and the person who had entered was recorded entering again. The operator is now told which way the pass went.
- The gate report counts the whole period instead of its first thousand rows, and the muster list asks the database once instead of once per person: 598 people took 1129 queries and 684 ms, and now take 5 and 109 ms.
- SNILS and INN are checked where a human types them. Both fields accepted anything up to 32 characters, and a stand record holds twenty digits in each to prove it. The SNILS check reuses the service that already knew how; the INN needed its own, because the twelve-digit form has two check digits and verifying only the last one lets exactly the kind of typo people make when copying from a document through.
- A correction to a name travels from any card that shows it. The person is the single place it is written, and the copies on the teacher and student cards are filled from there rather than being a second truth.
