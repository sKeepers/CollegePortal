...
<template>
  <AppPage>
    ...
    <template v-else>
      ...
      <div class="student-cabinet-grid">
        <AppCard title="Оценки" subtitle="Нажмите оценку, чтобы увидеть подробности.">
          <div v-if="store.grades.length" class="student-cabinet-grades"><button v-for="grade in store.grades" :key="grade.id" type="button" @click="openGrade(grade)"><strong>{{ grade.grade }}</strong><span>{{ grade.schedule_lesson?.subject?.name || 'Дисциплина' }}</span><small>{{ formatMobileDate(grade.schedule_lesson?.lesson_date) }}</small></button></div>
        ...
        <AppCard title="Посещаемость" :subtitle="`Присутствие: ${store.attendanceSummary.present}/${store.attendanceTotal}`">
          <div v-if="store.attendance.length" class="student-cabinet-attendance"><div v-for="item in store.attendance" :key="item.id"><strong>{{ item.schedule_lesson?.subject?.name || 'Занятие' }}</strong><span>{{ formatMobileDate(item.schedule_lesson?.lesson_date) }} · {{ attendanceLabel(item.status) }}<template v-if="item.status === 'late' && item.minutes_late"> · {{ item.minutes_late }} мин.</template></span></div></div>
    ...
    <q-dialog v-model="gradeDialogOpen" @hide="selectedGrade = null"><q-card class="student-cabinet-grade-dialog"><q-card-section><div class="text-h6">Оценка {{ selectedGrade?.grade || '—' }}</div></q-card-section><q-card-section class="student-cabinet-grade-dialog__details"><div><span>Дисциплина</span><strong>{{ selectedGrade?.schedule_lesson?.subject?.name || 'Не указана' }}</strong></div><div><span>Дата занятия</span><strong>{{ formatMobileDate(selectedGrade?.schedule_lesson?.lesson_date) }}</strong></div><div><span>Преподаватель</span><strong>{{ selectedGrade?.schedule_lesson?.teacher?.full_name || [selectedGrade?.schedule_lesson?.teacher?.last_name, selectedGrade?.schedule_lesson?.teacher?.first_name, selectedGrade?.schedule_lesson?.teacher?.middle_name].filter(Boolean).join(' ') || 'Не указан' }}</strong></div><div><span>Тип оценки</span><strong>{{ selectedGrade?.grade_type?.name || selectedGrade?.grade_type || 'Не указан' }}</strong></div><div v-if="selectedGrade?.comment"><span>Комментарий</span><strong>{{ selectedGrade.comment }}</strong></div></q-card-section><q-card-actions align="right"><q-btn flat label="Закрыть" v-close-popup /></q-card-actions></q-card></q-dialog>
  </AppPage>
</template>
