...
const canCreate = computed(() => permissions.hasPermission("exams.edit"));
const canUpdate = computed(() => permissions.hasPermission("exams.edit"));
const canDelete = computed(() => permissions.hasPermission("exams.edit"));
const canImport = computed(() => permissions.hasPermission("exams.edit"));
const canExport = computed(() =>
  permissions.hasAnyPermission(["exams.view", "exams.edit"]),
);
...
const syncingQuery = ref(false),
  ...
  resultDeleteDialogVisible = ref(false);
const editingExam = ref(null),
  editingResult = ref(null),
  deletingExam = ref(null),
  deletingResult = ref(null),
  importFile = ref(null);
...
function examTypeLabel(value) {
  return referenceOptions.label("exam_types", value, value || "—");
}
function examTypeTone(value) {
  return referenceOptions.tone("exam_types", value, "neutral");
}
const selectedStudentOptions = computed(() =>
  store.selectedExam?.group_id
    ? store.studentOptions.filter(
        (student) =>
          Number(student.group_id) === Number(store.selectedExam.group_id),
      )
...
const journalRoute = computed(() => ({
  path: "/journal",
  query: {
    group: store.selectedExam?.group_id,
    subject: store.selectedExam?.subject_id,
  },
}));
...
const examActions = computed(() => [
  ...
  {
    label: "Аудитория",
    to: classroomRoute.value,
    disabled: !store.selectedExam?.classroom_id,
  },
  { label: "Журнал", to: journalRoute.value },
]);
function timeRange(exam) {
  return (
    [exam?.starts_at, exam?.ends_at].filter(Boolean).join("–") ||
    "Время не указано"
  );
}
...
async function saveExam() {
  if (!canUpdate.value) return;
  if (!canUpdate.value) return;
  const isEdit = Boolean(editingExam.value?.id);
  await store.save(examForm, editingExam.value?.id || null);
  formVisible.value = false;
  notify(isEdit ? "Экзамен обновлен" : "Экзамен создан");
}
...
async function confirmDelete() {
  await store.remove(deletingExam.value);
  deletingExam.value = null;
  notify("Экзамен удален");
}
...
async function saveResult() {
  if (!canUpdate.value) return;
  if (!canUpdate.value) return;
  const isEdit = Boolean(editingResult.value?.id);
  await store.saveResult(resultForm);
  resultFormVisible.value = false;
  notify(isEdit ? "Результат обновлен" : "Результат добавлен");
}
function requestResultDelete(result) {
  deletingResult.value = result;
  resultDeleteDialogVisible.value = true;
}
async function confirmResultDelete() {
  await store.removeResult(deletingResult.value);
  deletingResult.value = null;
  notify("Результат удален");
}
...
async function handleImport(file) {
  ...
  notify("Импорт экзаменов завершен");
}
...
onMounted(async () => {
  await referenceOptions.loadCatalog("exam_types");
  store.selectById(routeSelectedId());
  await store.load();
  if (!store.selectedExam && store.filteredExams[0])
    await selectExam(store.filteredExams[0]);
});
</script>
...
<template>
  <AppPage>
    <PageHeader
      title="Экзамены и ГИА"
      subtitle="Экзамены, зачеты, ГИА и результаты студентов."
      ><template #actions
        ><q-btn v-if="canCreate" color="primary" @click="openCreateForm"
          ><Plus :size="16" class="q-mr-xs" /> Новый экзамен</q-btn
        ></template
      ></PageHeader
    >
    <AppToolbar
      ...
      ><template #actions
        ><AppLoading v-if="store.loading" label="Загрузка экзаменов..." /><q-btn
          flat
    ...
    <AppFilterBar
      ><q-select
        v-model="store.filters.academic_year"
        dense
        outlined
        clearable
        emit-value
        map-options
        label="Учебный год"
        :options="store.academicYearOptions"
      /><q-select
      ...
      /><q-select
        v-model="store.filters.teacher_id"
        dense
        outlined
        clearable
        emit-value
      ...
      /><q-select
        v-model="store.filters.exam_type"
        dense
        outlined
        clearable
        emit-value
    ...
    <div ref="workspaceRef" class="exams-workspace resizable-workspace" :style="workspaceStyle">
      <div class="exams-main">
        <AppTable
          ...
          ><template #body-cell-date="props"
            ><q-td :props="props"
              ><button
                class="exams-row-link"
                type="button"
                @click.stop="selectExam(props.row)"
              >
                {{ formatRuDate(props.row.exam_date) }}
              </button>
              <div class="exams-secondary-cell">
                {{ timeRange(props.row) }}
              </div></q-td
            ></template
          ...
          ><template #body-cell-subject="props"
            ><q-td :props="props"
              ><div class="exams-strong-cell">
                {{ subjectName(props.row.subject) }}
              </div>
              <div class="exams-secondary-cell">
                {{ props.row.topic || "Тема не указана" }}
              </div></q-td
            ></template
          ><template #body-cell-teacher="props"
            ><q-td :props="props">{{
              teacherName(props.row.teacher)
            }}</q-td></template
          ><template #body-cell-type="props"
            ><q-td :props="props"
              ><AppStatusBadge
                :label="examTypeLabel(props.row.exam_type)"
                :tone="examTypeTone(props.row.exam_type)" /></q-td></template
          ><template #body-cell-status="props"
            ><q-td :props="props"
              ><AppStatusBadge
                :label="examStatusLabel(props.row.status)"
                :tone="examStatusTone(props.row.status)" /></q-td></template
          ><template #body-cell-results="props"
            ><q-td :props="props">{{
              props.row.results_count ?? props.row.results?.length ?? 0
            }}</q-td></template
      ...
      <aside class="exams-side">
        <AppEmptyState
          ...
          description="Выберите строку в таблице, чтобы открыть карточку экзамена."
        /><WorkspacePanel
          v-else
          class="exams-card"
          :title="subjectName(store.selectedExam.subject)"
          :subtitle="`${store.selectedExam.group?.name || 'Группа не указана'} · ${formatRuDate(store.selectedExam.exam_date)}`"
          :metrics="examMetrics"
          :actions="examActions"
          ><template #status
            ><AppStatusBadge
              :label="examTypeLabel(store.selectedExam.exam_type)"
              :tone="
                examTypeTone(store.selectedExam.exam_type)
              " /><AppStatusBadge
              :label="examStatusLabel(store.selectedExam.status)"
              :tone="examStatusTone(store.selectedExam.status)"
          /></template>
          <div class="exams-details">
            <section>
              <h3>Основное</h3>
              <dl class="exams-fields">
                <div>
                  <dt>Преподаватель</dt>
                  <dd>{{ teacherName(store.selectedExam.teacher) }}</dd>
                </div>
                <div>
                  <dt>Аудитория</dt>
                  <dd>{{ classroomName(store.selectedExam.classroom) }}</dd>
                </div>
                <div>
                  <dt>Тема</dt>
                  <dd>{{ store.selectedExam.topic || "Не указана" }}</dd>
                </div>
              </dl>
            </section>
            <section>
              <div class="exams-section-header">
                ...
                <q-btn
                  ...
                  ><Plus :size="15" class="q-mr-xs" /> Добавить</q-btn
                >
              </div>
              <AppTable
                v-if="store.selectedResults.length"
                class="exams-results-table"
                :rows="store.selectedResults"
                ...
                ><template #body-cell-student="props"
                  ><q-td :props="props"
                    ...
                    >
                    <div v-if="props.row.comment" class="exams-secondary-cell">
                      {{ props.row.comment }}
                    </div></q-td
                  ></template
                ><template #body-cell-result="props"
                  ><q-td :props="props"
                    ><strong>{{ props.row.result || "—" }}</strong></q-td
                  ></template
                ...
                ><template #body-cell-status="props"
                  ><q-td :props="props"
                    ><AppStatusBadge
                      :label="resultStatusLabel(props.row.status)"
                      :tone="
                        resultStatusTone(props.row.status)
                      " /></q-td></template
                ><template #body-cell-actions="props"
                  ><q-td :props="props"
                    ><q-btn
                      ...
                      dense
                      title="Редактировать"
                      @click="openResultForm(props.row)"
                      ><Edit3 :size="15" /></q-btn
                    ><q-btn
                      flat
                      round
                      ...
                      color="negative"
                      title="Удалить"
                      @click="requestResultDelete(props.row)"
                      ><Trash2 :size="15" /></q-btn></q-td></template
              ></AppTable>
              <p v-else class="exams-muted">
                Результаты по выбранному экзамену пока не внесены.
              </p>
            </section>
          </div></WorkspacePanel
        >
    ...
    <q-dialog v-model="formVisible"
      ><q-card class="exams-dialog"
        ><q-card-section
          ><div class="text-h6">
            {{ editingExam ? "Редактировать экзамен" : "Новый экзамен" }}
          </div></q-card-section
        ><q-card-section class="exams-dialog__body"
          ><q-input
            v-model="examForm.academic_year"
            outlined
            dense
            label="Учебный год" /><q-input
            v-model="examForm.semester"
            outlined
            dense
            type="number"
            label="Семестр" /><q-select
            v-model="examForm.group_id"
            outlined
            dense
            emit-value
            map-options
            ...
            map-options
            label="Дисциплина"
            :options="store.subjectOptions" /><q-select
            v-model="examForm.teacher_id"
            outlined
            dense
            emit-value
            map-options
            label="Преподаватель"
            :options="store.teacherOptions" /><q-select
            v-model="examForm.classroom_id"
            outlined
            dense
            clearable
            emit-value
            map-options
            label="Аудитория"
            :options="store.classroomOptions" /><q-input
            ...
            map-options
            label="Тип"
            :options="examTypeOptions" /><q-select
            ...
            map-options
            label="Статус"
            :options="EXAM_STATUS_OPTIONS" /><q-input
            v-model="examForm.topic"
            outlined
            dense
            label="Тема / примечание" /></q-card-section
        ><q-card-actions align="right"
          ><q-btn flat label="Отмена" @click="formVisible = false" /><q-btn
            color="primary"
            :loading="store.saving"
            :disable="
              !examForm.academic_year ||
              !examForm.group_id ||
              !examForm.subject_id ||
              !examForm.teacher_id ||
              !examForm.exam_date
      ...
      ></q-dialog
    >
    <q-dialog v-model="resultFormVisible"
      ><q-card class="exams-dialog"
        ><q-card-section
          ><div class="text-h6">
            {{
        ...
        ><q-card-section class="exams-dialog__body"
          ><q-select
            ...
            :disable="Boolean(editingResult)" /><q-select
            v-model="resultForm.result"
            outlined
            dense
            clearable
            label="Оценка / отметка"
            :options="RESULT_OPTIONS" /><q-input
            v-model="resultForm.score"
            outlined
            dense
            type="number"
            label="Баллы" /><q-select
            ...
            map-options
            label="Статус"
            :options="RESULT_STATUS_OPTIONS" /><q-input
        ...
        ><q-card-actions align="right"
          ><q-btn
            flat
            label="Отмена"
            @click="resultFormVisible = false"
          /><q-btn
            color="primary"
            :loading="store.saving"
            :disable="!resultForm.student_id"
            @click="saveResult"
            >Сохранить</q-btn
          ></q-card-actions
        ></q-card
    ...
    <AppConfirmDialog
      ...
      :message="
        deletingExam
          ? `Будет удален экзамен: ${subjectName(deletingExam.subject)} (${formatRuDate(deletingExam.exam_date)}).`
          : 'Будет удален выбранный экзамен.'
      "
    ...
    />
    <AppConfirmDialog
      v-model="resultDeleteDialogVisible"
      title="Удалить результат?"
      :message="
        deletingResult
          ? `Будет удален результат студента: ${studentName(deletingResult.student)}.`
          : 'Будет удален выбранный результат.'
      "
      confirm-label="Удалить"
      tone="negative"
      @confirm="confirmResultDelete"
    />
  </AppPage>
</template>
