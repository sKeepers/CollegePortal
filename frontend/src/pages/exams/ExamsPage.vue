...
const canExport = computed(() => permissions.hasAnyPermission(['exams.view', 'exams.edit']))
...
function openCreateForm() { if (!canCreate.value) return; editingExam.value = null; Object.assign(examForm, { academic_year: '2026/2027', semester: 1, group_id: '', subject_id: '', teacher_id: '', classroom_id: '', exam_date: new Date().toISOString().slice(0, 10), starts_at: '09:00', ends_at: '10:30', exam_type: 'exam', status: 'scheduled', topic: '' }); formVisible.value = true }
function openEditForm(exam) { if (!canUpdate.value) return; editingExam.value = exam; Object.assign(examForm, { academic_year: exam.academic_year, semester: exam.semester, group_id: exam.group_id, subject_id: exam.subject_id, teacher_id: exam.teacher_id, classroom_id: exam.classroom_id || '', exam_date: exam.exam_date, starts_at: exam.starts_at || '', ends_at: exam.ends_at || '', exam_type: exam.exam_type || 'exam', status: exam.status || 'scheduled', topic: exam.topic || '' }); formVisible.value = true }
async function saveExam() { if (!canUpdate.value) return; if (!canUpdate.value) return; const isEdit = Boolean(editingExam.value?.id); await store.save(examForm, editingExam.value?.id || null); formVisible.value = false; notify(isEdit ? 'Экзамен обновлен' : 'Экзамен создан') }
function requestDelete(exam) { if (!canDelete.value) return; deletingExam.value = exam; deleteDialogVisible.value = true }
...
async function saveResult() { if (!canUpdate.value) return; if (!canUpdate.value) return; const isEdit = Boolean(editingResult.value?.id); await store.saveResult(resultForm); resultFormVisible.value = false; notify(isEdit ? 'Результат обновлен' : 'Результат добавлен') }
...
async function handleImport(file) { if (!canImport.value || !file) return; await store.importCsv(file); importFile.value = null; notify('Импорт экзаменов завершен') }
async function exportCsv() { if (!canExport.value) return; await store.exportCsv(); notify('Экспорт экзаменов подготовлен') }
...
<template>
  <AppPage>
    <PageHeader title="Экзамены и ГИА" subtitle="Экзамены, зачеты, ГИА и результаты студентов."><template #actions><q-btn v-if="canCreate" color="primary" @click="openCreateForm"><Plus :size="16" class="q-mr-xs" /> Новый экзамен</q-btn></template></PageHeader>
