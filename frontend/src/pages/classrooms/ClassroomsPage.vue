...
async function syncClassroomQuery({ selectedId = routeSelectedId(), searchText = routeSearchText() }) {
  ...
  await router.replace({ path: '/classrooms', query })
  ...
  <AppPage>
    ...
    <AppToolbar>
      ...
      <template #actions>
        ...
        <q-btn color="secondary" :disable="store.loading" @click="exportClassrooms">
          <template #default>
            <Download :size="16" />
            <span>Экспорт</span>
    ...
    <div class="classrooms-layout">
      <div class="classrooms-main">
        ...
        >
          <q-btn color="primary" label="Новая аудитория" @click="openCreateForm" />
        </AppEmptyState>
      </div>
