...
<!--
  ...
  Блок НЕ scoped, и это обязательно: `body *` со скоупом превратился бы в
  `body *[data-v-...]` и спрятал бы только свои же узлы, а не страницу.

  Обратная сторона — то, из-за чего здесь стоит `!important`. Правила соседнего
  scoped-блока получают тот самый `[data-v-...]`, то есть вес выше здешних, и
  побеждают независимо от порядка. Замерено в браузере 28.08.2026: на ширине
  1600 сетка давала шесть колонок вместо трёх, а вычисленный цвет рамки был
  `rgb(153, 153, 153)` — базовый #999 вместо чёрного. Колонки на A4 совпадают
  случайно (240px минимум даёт те же три), а вот серая линия реза уезжала на
  бумагу по-настоящему. Правило, которое не применяется, хуже отсутствующего:
  оно останавливает того, кто пришёл проверить.
-->
<style>
@media print {
  ...
  .account-card-grid { grid-template-columns: repeat(3, 1fr) !important; }
  .account-card { border-color: #000 !important; }
}
</style>
