...
const tableSubtitle = computed(() => {
  // Пока список не пришёл, числа у нас нет — и «Найдено записей: 0» человек
  // читает как «студентов нет». На стенде список отвечает 8–11 секунд, и всё
  // это время на экране стоял ноль. Третий случай к нашему правилу о пустом
  // состоянии: не «данных нет» и не «спросить не удалось», а «ещё не знаю».
  if (store.loading) {
    return 'Ищем записи…'
  }
...
<template>
  <AppPage>
    ...
    <!--
      Пустой фильтр без объяснения — та же неизвестность, что и пустой список:
      человек не поймёт, нет ли групп или их не дали. Строка появляется только
      когда справочник действительно отказал.
    -->
    <q-banner v-if="store.groupsUnavailable" dense rounded class="bg-amber-1 text-amber-10 q-mb-sm">
      Справочник групп недоступен для вашей роли — отбор по группе, курсу и специальности не работает.
      Список студентов это не затрагивает.
    </q-banner>
...
<!--
  Правил `@media print` у страницы нет намеренно, и возвращать их нельзя.

  Карточки печатаются отдельным документом (`printCredentials` →
  `printHtmlDocument`), а лист, собранный из самой страницы, теряет карточки
  молча: пока окно открыто, Quasar держит `body` в `position: fixed` через класс
  `q-body--prevent-scroll`, у документа нет потока, и печать даёт ровно один лист.

  Замерено 29.08.2026 на 27 карточках: Ctrl+P при открытом окне — одна страница,
  на ней 21 карточка из 27, остальное приложение скрыто, и лист выглядит целым.
  Тот же замер со снятым классом и ничем больше — две страницы. Именно правила
  печати делали обрезанный лист правдоподобным, поэтому они удалены, а не
  исправлены. Сторож: `PagesDoNotPrintThemselvesTest`.
-->
