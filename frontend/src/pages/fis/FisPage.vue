...
const outboundSummary = computed(() => store.outboundSummary || { total: 0, ready: 0, blocked: true })
const gatewayCards = computed(() => [
  { label: 'Gateway Agent', value: store.gatewayStatus.health?.ok ? 'доступен' : 'недоступен', tone: store.gatewayStatus.health?.ok ? 'success' : 'danger', detail: store.gatewayStatus.health?.message || 'Не проверен' },
  { label: 'Версия агента', value: store.gatewayStatus.version?.gateway_version || store.gatewayStatus.version?.message || '—', tone: store.gatewayStatus.version?.ok ? 'info' : 'neutral', detail: 'GET /version' },
...
async function runGateway(action, label) { await action(); notify(label) }
...
<template>
  <AppPage>
    ...
    <q-banner class="fis-outbound-banner" rounded>
      ...
      <div><strong>Официальный outbound-коннектор ФИС</strong><span>Исходящие пакеты: {{ outboundSummary.total }} · XSD: {{ store.specInfo?.xsd_loaded ? 'загружена' : 'ожидает официальной спецификации' }} · Production заблокирован.</span></div>
    </q-banner>

    <section class="fis-gateway-panel">
      <div class="fis-gateway-panel__header">
        <div><h2>Шлюз ФИС</h2><p>Диагностика защищенного Gateway Agent на ViPNet-ПК. Validate/Import отключены до загрузки официальной прикладной XSD пакета ФИС.</p></div>
        <div class="fis-gateway-panel__actions"><q-btn dense outline :loading="store.saving" @click="runGateway(store.checkGatewayHealth, 'Шлюз проверен')">Проверить шлюз</q-btn><q-btn dense outline :loading="store.saving" @click="runGateway(store.checkGatewayVersion, 'Версия получена')">Версия агента</q-btn><q-btn dense color="primary" :loading="store.saving" @click="runGateway(store.checkZkspd, 'ЗКСПД проверен')">Проверить ЗКСПД</q-btn><q-btn dense outline :loading="store.saving" @click="runGateway(store.loadGatewayDictionaries, 'Запрошен тестовый список справочников')">Тестовые справочники</q-btn><q-btn dense outline :loading="store.saving" @click="runGateway(store.loadGatewayInstitution, 'Запрошены сведения организации')">Сведения организации</q-btn><q-btn dense disable color="warning">Validate отключен</q-btn><q-btn dense disable color="negative">Import отключен</q-btn></div>
      </div>
...
.fis-gateway-panel { margin: 12px 0; padding: 16px; border: 1px solid #d9e2ec; border-radius: 8px; background: #fff; }
.fis-gateway-panel__header { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; }
.fis-gateway-panel__header h2 { margin: 0 0 4px; font-size: 18px; }
.fis-gateway-panel__header p { margin: 0; color: #64748b; }
.fis-gateway-panel__actions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-end; }
.fis-gateway-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 10px; margin-top: 14px; }
.fis-gateway-card { border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px; display: grid; gap: 6px; min-height: 94px; }
.fis-gateway-card span { font-weight: 700; color: #334155; }
.fis-gateway-card small { color: #64748b; line-height: 1.35; }
