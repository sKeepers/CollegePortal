...
const curriculumMetrics = computed(() => [
  { label: 'Код', value: store.selectedCurriculum?.code || '—' },
  { label: 'Год', value: store.selectedCurriculum?.year_start || '—' },
  { label: 'Дисциплин', value: store.selectedItems.length },
...
function openCreateForm() { if (!canCreate.value) return; editingCurriculum.value = null; codeEditable.value = false; Object.assign(curriculumForm, { code: '', education_program_id: '', name: '', year_start: new Date().getFullYear(), status: 'draft', description: '' }); formVisible.value = true }
...
async function saveCurriculum() { if (!canUpdate.value) return; if (!canUpdate.value) return; const isEdit = Boolean(editingCurriculum.value?.id); await store.save(curriculumForm, editingCurriculum.value?.id || null); formVisible.value = false; notify(isEdit ? 'Учебный план обновлен' : 'Учебный план создан') }
...
function openItemForm() { if (!canUpdate.value) return; Object.assign(itemForm, { subject_id: '', course: 1, semester: 1, hours_total: 0, control_form: 'Зачет', sort_order: store.selectedItems.length + 1 }); itemFormVisible.value = true }
async function addItem() { if (!canUpdate.value) return; if (!canUpdate.value) return; await store.addItem(itemForm); itemFormVisible.value = false; notify('Дисциплина добавлена в учебный план') }
...
<template>
  <AppPage>
    <PageHeader title="Учебные планы" subtitle="Планы образовательных программ, дисциплины по курсам и семестрам, часы и формы контроля.">
      <template #actions><q-btn v-if="canCreate" color="primary" @click="openCreateForm"><Plus :size="16" class="q-mr-xs" /> Новый план</q-btn></template>
    </PageHeader>
    ...
    <div class="curricula-workspace">
      ...
      <aside class="curricula-side"><AppEmptyState v-if="!store.selectedCurriculum" title="Учебный план не выбран" description="Выберите строку в таблице, чтобы открыть состав плана." /><WorkspacePanel v-else class="curricula-card" :title="store.selectedCurriculum.name" :subtitle="programName(store.selectedCurriculum)" :metrics="curriculumMetrics" :actions="curriculumActions"><template #status><AppStatusBadge :label="statusLabel(store.selectedCurriculum.status)" :tone="statusTone(store.selectedCurriculum.status)" /></template><div class="curricula-details"><section><h3>Основное</h3><dl class="curricula-details__list"><div><dt>Специальность</dt><dd>{{ specialtyName(store.selectedCurriculum) }}</dd></div><div><dt>Код</dt><dd>{{ store.selectedCurriculum.code || '—' }}</dd></div><div><dt>Программа</dt><dd>{{ programName(store.selectedCurriculum) }}</dd></div><div><dt>Описание</dt><dd>{{ store.selectedCurriculum.description || '—' }}</dd></div></dl></section><section><div class="curricula-section-header"><h3>Дисциплины</h3><q-btn dense color="primary" :disable="store.saving" @click="openItemForm"><BookPlus :size="15" class="q-mr-xs" /> Добавить</q-btn></div><AppTable v-if="store.selectedItems.length" :rows="store.selectedItems" :columns="itemColumns" :pagination="{ rowsPerPage: 0 }" :rows-per-page-options="[0]"><template #body-cell-subject="props"><q-td :props="props">{{ subjectName(props.row) }}</q-td></template><template #body-cell-actions="props"><q-td :props="props"><q-btn v-if="canUpdate" flat round dense color="negative" title="Удалить" @click="requestItemDelete(props.row)"><Trash2 :size="15" /></q-btn></q-td></template></AppTable><p v-else class="curricula-muted">Дисциплины пока не добавлены.</p></section></div></WorkspacePanel></aside>
    </div>
