...
async function loadDashboard() {
  ...
  try {
    ...
    const teachersResult = !isAdmission.value && auth.can('teachers.view') ? await api.listAll('teachers', { active_only: 1 }).then((value) => ({ status: 'fulfilled', value })).catch((reason) => ({ status: 'rejected', reason })) : null
