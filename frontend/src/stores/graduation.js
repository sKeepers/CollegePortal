...
export const useGraduationStore = defineStore('graduation', () => {
  ...
  async function load() { loading.value = true; error.value = ''; try { const [graduatesPayload, studentsPayload, groupsPayload, programsPayload, specialtiesPayload] = await Promise.all([api.list('graduates'), api.list('students'), api.list('groups'), api.list('education-programs'), api.list('specialties')]); graduates.value = extractRows(graduatesPayload); students.value = extractRows(studentsPayload); groups.value = extractRows(groupsPayload); programs.value = extractRows(programsPayload); specialties.value = extractRows(specialtiesPayload); if (selectedId.value && !selectedGraduate.value) selectedId.value = null } catch (err) { error.value = err.message || 'Не удалось загрузить выпускников' } finally { loading.value = false } }
