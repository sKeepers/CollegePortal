...
export const useGraduationStore = defineStore('graduation', () => {
  ...
  async function load() { loading.value = true; error.value = ''; try { const { payloads } = await loadReferences({ graduates: api.listAll('graduates'), students: api.listAll('students'), groups: api.listAll('groups'), programs: api.listAll('education-programs'), specialties: api.listAll('specialties') }); graduates.value = extractRows(payloads.graduates); students.value = extractRows(payloads.students); groups.value = extractRows(payloads.groups); programs.value = extractRows(payloads.programs); specialties.value = extractRows(payloads.specialties); if (selectedId.value && !selectedGraduate.value) selectedId.value = null } catch (err) { error.value = err.message || 'Не удалось загрузить выпускников' } finally { loading.value = false } }
