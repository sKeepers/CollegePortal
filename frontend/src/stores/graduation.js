...
export const useGraduationStore = defineStore('graduation', () => {
  ...
  async function load() { loading.value = true; error.value = ''; try { const { payloads } = await loadReferences({ graduates: api.list('graduates'), students: api.list('students'), groups: api.list('groups', { per_page: 200 }), programs: api.list('education-programs'), specialties: api.list('specialties') }); graduates.value = extractRows(payloads.graduates); students.value = extractRows(payloads.students); groups.value = extractRows(payloads.groups); programs.value = extractRows(payloads.programs); specialties.value = extractRows(payloads.specialties); if (selectedId.value && !selectedGraduate.value) selectedId.value = null } catch (err) { error.value = err.message || 'Не удалось загрузить выпускников' } finally { loading.value = false } }
