...
- Маршруты `api/fis/outbound/*` лежат внутри группы `permission:manage_dictionaries`, и это право выводится из префикса URL. Тестовому пользователю мало права на само действие: для POST нужен ещё и `fis.outbound.create`, иначе внешняя проверка группы даёт 403.
