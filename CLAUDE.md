...
- ФИО, телефон и email человека пишутся только через `PersonService`: карточки сотрудника и преподавателя правят Person, а свои копии получают зеркалом. Копия ФИО в `teachers` — не второй источник правды. `students` эту рассылку ещё не получил: правка карточки студента до человека не доходит.
