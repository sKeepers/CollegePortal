...
- Маршрутам `api/fis/outbound/*` на POST мало права на само действие: рядом объявлено ещё и `fis.outbound.create`, без него будет 403. Это наследство таблицы префиксов, где POST выводился в «создание», и при переносе права к маршруту оно сохранено дословно, чтобы ничего не расширить. Следствие видно на директоре: у него есть `fis.outbound.view` и `fis.outbound.status`, но нет `create`, поэтому проверки связи со шлюзом и обновление статуса ему закрыты. Пересмотреть — решение владельца.
